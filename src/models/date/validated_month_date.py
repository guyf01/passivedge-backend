"""ValidatedMonthDate dataclass with validation."""
...
class ValidatedMonthDate(MonthDate):
    """
    MonthDate with validation.
    ...
    def next_month(self) -> ValidatedMonthDate:
        """
        Get the next month.
        
        :return: New ValidatedMonthDate for next month
        """
        if self.month == 12:
            return ValidatedMonthDate(year=self.year + 1, month=1)
        else:
            return ValidatedMonthDate(year=self.year, month=self.month + 1)


    def generate_range_to(self, end: ValidatedMonthDate):
        """
